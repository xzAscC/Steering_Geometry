...
from steering_geometry.concepts.refusal import evaluate_refusal, load_refusal_data
...
def main() -> None:
    ...
    concept_config = ConceptConfig(
        ...
        dataset_name="synthetic_repe",
        num_pairs=args.num_pairs,
    )
    pairs = load_refusal_data(concept_config)
    ...
    print(f"Refusal shift: {evaluation.scores['refusal_shift']:.4f}")

    output_dir = Path(args.output)
    output_dir.mkdir(parents=True, exist_ok=True)
    safe_model_name = args.model.replace("/", "_")
    output_file = output_dir / f"refusal_{safe_model_name}_{args.method}.pt"
    torch.save(
        {
            "vector": vector,
