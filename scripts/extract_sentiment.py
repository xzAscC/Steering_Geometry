...
from steering_geometry.concepts.sentiment import evaluate_sentiment, load_sentiment_data
...
def main() -> None:
    ...
    pairs = load_sentiment_data(concept_config)
    ...
    evaluation = evaluate_sentiment(
        model=model,
        vector=vector,
        config=EvaluationConfig(),
    )
    print(f"Sentiment shift: {evaluation.scores['sentiment_shift']:.4f}")

    output_dir = Path(args.output)
    output_dir.mkdir(parents=True, exist_ok=True)
    safe_model_name = args.model.replace("/", "_")
    output_file = output_dir / f"sentiment_{safe_model_name}_{args.method}.pt"
    torch.save(
        {
            "vector": vector,
