...
from steering_geometry.concepts.honesty import evaluate_honesty, load_honesty_data
...
def main() -> None:
    ...
    pairs = load_honesty_data(concept_config)
    ...
    evaluation = evaluate_honesty(
        model=model,
        vector=vector,
        config=EvaluationConfig(),
    )
    print(f"Honesty shift: {evaluation.scores['honesty_shift']:.4f}")

    output_dir = Path(args.output)
    output_dir.mkdir(parents=True, exist_ok=True)
    safe_model_name = args.model.replace("/", "_")
    output_file = output_dir / f"honesty_{safe_model_name}_{args.method}.pt"
    torch.save(
        {
            "vector": vector,
