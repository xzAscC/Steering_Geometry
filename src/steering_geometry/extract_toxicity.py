...
from steering_geometry.concepts.toxicity import evaluate_toxicity, load_toxicity_data
...
def main() -> None:
    ...
    pairs = load_toxicity_data(concept_config)
    ...
    print(f"Toxicity shift: {evaluation.scores['toxicity_shift']:.4f}")

    output_dir = Path(args.output)
    output_dir.mkdir(parents=True, exist_ok=True)
    safe_model_name = args.model.replace("/", "_")
    output_file = output_dir / f"toxicity_{safe_model_name}_{args.method}.pt"
    torch.save(
        {
            "vector": vector,
